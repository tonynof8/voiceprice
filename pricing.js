// ============================================
// PRICING.JS - Логика расчётов (портировано с Python)
// ============================================

console.log('📦 Загрузка pricing.js...');

// ============================================
// УТИЛИТЫ ДЛЯ ОБРАБОТКИ ТЕКСТА
// ============================================

// Регулярные выражения (аналог Python)
const DASHES_RE = /[\u2010\u2011\u2012\u2013\u2014\u2212]/g;
...
// ============================================
// ПРИБЛИЗИТЕЛЬНЫЙ ПОДСЧЁТ СЛОВ В ЧИСЛЕ
// ============================================

function countWordsInNumber(num) {
  // Примерный подсчёт количества слов при произнесении числа
  num = Math.abs(parseInt(num));
  
  if (num === 0) return 1; // "ноль"
  if (num <= 20) return 1;
  if (num <= 99) return 2; // "двадцать один"
  if (num <= 999) {
    // 100 = "сто" (1 слово)
    // 101 = "сто один" (2 слова)
    // 999 = "девятьсот девяносто девять" (3 слова)
    let words = 1; // сотни
  ...
  if (num <= 9999) {
    // 1000 = "одна тысяча" (2 слова)
    // 1891 = "одна тысяча восемьсот девяносто один" (5 слов)
    let thousands = Math.floor(num / 1000);
    let remainder = num % 1000;
    let words = countWordsInNumber(thousands) + 1; // число + "тысяча"
    if (remainder > 0) {
      words += countWordsInNumber(remainder);
    }
    return words;
  ...
  if (num <= 999999) {
    ...
    let thousands = Math.floor(num / 1000);
    let remainder = num % 1000;
    let words = countWordsInNumber(thousands) + 1; // + "тысяч"
    if (remainder > 0) {
      words += countWordsInNumber(remainder);
    }
    return words;
  }
  // Для больших чисел - приблизительно
  return Math.floor(num.toString().length / 2) + 2;
}

// ============================================
// ПОДСЧЁТ СЛОВ (учитывая числа)
// ============================================

function countWords(text) {
  if (!text) return 0;
  
  // 1) Нормализуем странные дефисы и пробелы
  text = text.replace(DASHES_RE, '-');
  text = text.replace(NON_BREAK_SPACES_RE, ' ');
  text = text.replace(ZERO_WIDTH_RE, '');
  
  // 2) СЛОВАРЬ СИМВОЛОВ (из utils.py replace_symbols_with_words)
  ...
  // 6) Заменяем дефисы на пробелы (чтобы "стрелять-колотить" = 2 слова)
  text = text.replace(/-/g, ' ');
  
  // 7) Выбрасываем лишние символы (оставляем только буквы, цифры, пробелы)
...
// ============================================
// ФУНКЦИЯ ОКРУГЛЕНИЯ (из utils.py round_to_hundred)
// ============================================
...
// ============================================
// НАЦЕНКА ЗА СРОЧНОСТЬ (из utils.py apply_urgent_markup)
// ============================================
...
// ============================================
// РАСЧЁТ ДЕДЛАЙНА (из utils.py calculate_deadline)
// ============================================

function calculateDeadline(durationMinutes, isUrgent = false, extraDays = 0) {
  let today = new Date();
  const weekday = today.getDay(); // 0=Вс, 1=Пн, ..., 6=Сб
  
  // JavaScript: 0=Вс, Python: 0=Пн => конвертируем
  const pythonWeekday = weekday === 0 ? 6 : weekday - 1;
  
  // Если выходной (>=5 значит Сб или Вс) и не срочный, переносим на понедельник
  if (pythonWeekday >= 5 && !isUrgent) {
    const daysUntilMonday = 7 - pythonWeekday;
    today.setDate(today.getDate() + daysUntilMonday);
  }
  
  // Срочный заказ до 30 минут
  ...
  // Подсчёт рабочих дней (пропускаем выходные)
...
// ============================================
// ОПТИМАЛЬНЫЙ ХРОНОМЕТРАЖ (из utils.py get_optimal_timing)
// ============================================
...
function calculatePrice(service, value, isUrgent = false) {
  ...
  switch (service) {
    ...
    case 'translate_voice':
      ...
      deadline = calculateDeadline(durationMinutes, isUrgent, 3); // +3 дня для перевода
      break;
...
console.log('✅ pricing.js загружен');