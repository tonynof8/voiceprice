...
async function countBackend(text) {
  ...
  const url = service === 'translate_text'
    ? "https://tonynof8.onrender.com/count_chars"
    : "https://tonynof8.onrender.com/count_words";
...
async function calculate() {
  ...
  try {
    ...
    const response = await fetchWithRetry(
      "https://tonynof8.onrender.com/calculate",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
(async function warmupServer() {
  try {
    ...
    await fetch('https://tonynof8.onrender.com/calculate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
